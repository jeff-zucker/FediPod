...
| **Changing where the address lives** | the shape is chosen at sign-up (`signup.mjs`): on the pod, `@you@yourpod` with the gateway as a mail door, or at the gateway, `@you@front` — and a suffixed pod is always fronted <!-- CLAUDE 2026-09-25 — was "a pod on a suffix-based host"; delete this marker when done -->. `admin-facade.mjs` refuses changing it afterwards, because a rename needs a restart a browser does not have. |
