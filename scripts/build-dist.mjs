...
//
//   node scripts/build-dist.mjs        → dist/activitypod-js-<version>.tar.gz
//
// Unpack-and-go:  tar xzf activitypod-js-*.tar.gz && cd activitypod-js
...
const stage = path.join(root, 'dist', 'stage', 'activitypod-js');
const out = path.join(root, 'dist', `activitypod-js-${pkg.version}.tar.gz`);
...
execFileSync('tar', ['czf', out, '-C', path.dirname(stage), 'activitypod-js']);
fs.rmSync(path.join(root, 'dist', 'stage'), { recursive: true, force: true });
