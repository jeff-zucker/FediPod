...
| `plans/pod-as-relay.md` | the pod as a relay: `privateRoot` moves the RDF truth and the operational state to a pod on your machine, the pod keeps only what the protocol forces to be public (plus the lease, pinned deliberately), and why the inbox drain now writes before it deletes. Diagram: `diagrams/architecture-relay.svg`. |
