...
| `plans/pod-as-relay.md` | the pod as a relay: `privateRoot` moves the RDF truth and the operational state to a pod or a directory of your own, the pod keeps only what the protocol forces to be public (plus the lease, pinned deliberately), why the inbox drain now writes before it deletes, and why `lib/storage.mjs` is four operations rather than an LDP implementation. Diagram: `diagrams/architecture-relay.svg`. |
