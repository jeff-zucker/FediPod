...
| `plans/pod-as-relay.md` | the pod as a relay: `privateRoot` moves the RDF truth and the operational state to a pod or a directory of your own, the pod keeps only what the protocol forces to be public (plus the lease, pinned deliberately), why the inbox drain now writes before it deletes, why `lib/storage.mjs` is four operations rather than an LDP implementation, and how a machine that is behind the pod catches up — followers, the outbox, and `activitypod rebuild` for your own posts. Diagram: `architecture-relay.svg`, at repo root. |
...
- **`drafts/`** is gitignored and holds the security notes plus the superseded
  architecture diagrams (`architecture.png`, `architecture2.png`, and since
  2026-08-03 `architecture.svg` — the current one is `architecture-relay.svg` at
  repo root); leave it alone.
