...
import { identityHomes, rootOf } from './home.mjs';
...
const OPEN_POSTS = new Set(['/block', '/unblock', '/setup', '/setup/check']);
...
const LOCAL_ONLY_POSTS = new Set(['/setup', '/setup/check', '/config', '/new-actor', '/start-actor']);
...
// Bind to find out, the way `activitypod up` does: something silently holding a
// port without answering HTTP reads as free to a probe, and the new agent then
// dies on EADDRINUSE with nobody watching.
function portFree(p) {
    ...
    try {
      ...
      if (req.method === 'GET' && p === '/profiles') {
        ...
        const rows = await Promise.all(identityHomes(rootOf(here)).map(async ({ name, dir }) => {
          let port = null;
          try { port = JSON.parse(fs.readFileSync(path.join(dir, 'agent.json'), 'utf8')).port || null; } catch {}
          ...
          return {
            name, port, current,
            admin: port ? `http://localhost:${port}/admin/` : null,
            app: port ? `http://localhost:${port}/` : null,
      ...
      if (req.method === 'GET' || req.method === 'HEAD') {
        ...
        if (p === '/' || p === '/index.html') {
          const where = isGroup() ? '/admin/'
            : !(agent.home && hasCredential(agent.home)) ? SETUP_PAGE
              : null;
          if (where) {
        ...
        case '/new-actor': {
          ...
          if (started.status !== 202) {
            return json(res, started.status || 502, {
              ...
              port: newPort, url: `http://localhost:${newPort}${SETUP_PAGE}`,
            });
          }
          return json(res, 200,
            { ok: true, handle, port: newPort, url: `http://localhost:${newPort}${SETUP_PAGE}` });
        }
        ...
        case '/start-actor': {
          ...
          if (want) {
            const live = await fetch(`http://localhost:${want}/status`, { signal: AbortSignal.timeout(1000) })
              .then(r => r.ok).catch(() => false);
            if (live) return json(res, 200, { ok: true, name, port: want, url: `http://localhost:${want}/`, already: true });
          }
          ...
          for (let i = 0; i < 60; i++) {
            ...
            const up = await fetch(`http://localhost:${on}/status`, { signal: AbortSignal.timeout(1000) })
              .then(r => r.ok).catch(() => false);
            if (up) return json(res, 200, { ok: true, name, port: on, url: `http://localhost:${on}/` });
          }
