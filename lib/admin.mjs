...
import { allowedAuthorities, checkRequest, isCrossSiteNavigation, Authorities } from './guard.mjs';
import { newRun, preflight, runSetup, setupInputError, hasCredential, probeLocalPods } from './setup.mjs';
...
// Our own pages, kept out of ui/ for two reasons: dropping a client dist named
// "setup" there would otherwise shadow them, and a group serves these and
// nothing else — so the prefix has to be one nobody is invited to write into.
const WEB_DIR = path.join(projectRoot, 'web');
const WEB_MOUNTS = ['setup', 'admin'];
...
const OPEN_POSTS = new Set(['/block', '/setup', '/setup/check']);
...
const LOCAL_ONLY_POSTS = new Set(['/setup', '/setup/check', '/config']);
...
// republished.
const WIRE_CONFIG = ['name', 'summary', 'icon', 'approveJoins'];
...
export function startAdmin({ port, gateToken, agent, log = console.log, handle = null }) {
  ...
  const handler = async (req, res) => {
    ...
    try {
      ...
      // ---- setup and configuration, for the pages under /setup/ and /admin/ ----
      ...
      // Kept off /setup/state so that stays instant: this one goes out and
      // knocks on a couple of loopback ports.
      if (req.method === 'GET' && p === '/setup/local-pods') {
        return json(res, 200, { pods: await probeLocalPods() });
      }
      ...
      if (req.method === 'GET' && p === '/config') {
        ...
        if (!cfg) return json(res, 409, { error: 'agent not configured — set it up at /setup/' });
        ...
        return json(res, 200, {
          ...
          // which is `activitypod state --to <url>`.
          privateRoot: agent.readCredential?.()?.privateRoot || null,
          mode: agent.status?.().mode || null, port, home: agent.home || null,
          origins: {
            loopback: `http://localhost:${port}/`,
            named: allowed.label ? `http://${allowed.label}.localhost:${port}/` : null,
          },
      ...
      if (req.method === 'GET' || req.method === 'HEAD') {
        ...
        if (mount) {
          // Without the slash the page's own relative <script src> would
          // resolve one level up and 404.
          if (p === `/${mount}`) {
            res.writeHead(302, { location: `/${mount}/`, ...securityHeaders(allowed, false) });
            res.end();
            return;
          }
        ...
        if (p === '/' || p === '/index.html') {
          const where = isGroup() ? '/admin/'
            : !(agent.home && hasCredential(agent.home)) ? '/setup/'
              : null;
          if (where) {
            res.writeHead(302, { location: where, ...securityHeaders(allowed, false) });
            res.end();
            return;
          }
        }
      ...
      if (!OPEN_POSTS.has(p) && !agent.configured()) {
        return json(res, 409, { error: 'agent not configured — set it up at /setup/' });
      }
      if (LOCAL_ONLY_POSTS.has(p) && !allowed.isLocal(req.headers.host)) {
        return json(res, 403, { error: 'setup and configuration are available on this machine only' });
      }
      switch (p) {
        // ---- setup, driven by the page at /setup/ ----
        ...
        case '/config': {
          ...
          if (republish) await agent.publisher.publishProfile();
          return json(res, 200, {
  ...
  server.listen(port, '127.0.0.1', () => {
    ...
    log(`solid-activitypub on http://localhost:${port}/ (UI + API)`);
  });
