...
import { addRemoveActivity } from './wire.mjs';
...
const LOCAL_ONLY_POSTS = new Set(['/setup', '/setup/check', '/config', '/new-actor', '/start-actor', '/shutdown', '/state-move', '/atproto/connect', '/gateway']);
...
export function startAdmin({ port, gateToken, agent, log = console.log, handle = null }) {
  ...
  const c2s = new C2S({ agent, log, auth: makeC2sAuth({ agent, masto, log }) });
