...
const LOCAL_ONLY_POSTS = new Set(['/setup', '/setup/check', '/config', '/new-actor', '/start-actor', '/shutdown']);
...
export function startAdmin({ port, gateToken, agent, log = console.log, handle = null }) {
  ...
  const handler = async (req, res) => {
    ...
    try {
      ...
      // The detail goes to the log, where the operator can read it; the caller
      // gets a sentence. e.message here carries filesystem paths and internals.
      log(`admin ${p}: ${e.stack || e.message}`);
      return json(res, 500, { error: 'the agent failed to handle that — see the log' });
    }
  };
