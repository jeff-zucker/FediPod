...
function gatewayConfigFrom(fr) {
  ...
  return fr.fronted
    ? { url: target.replace(/ap\/actor$/, 'ap/inbox/'), frontActor: target,
      mode: 'trust', hmacSecret: String(fr.secret) }
    : { url: target.replace(/\/?$/, '/'), mode: 'trust', hmacSecret: String(fr.secret) };
}
...
// The https origin for an identity, on the mirrored port space (8030 → 9030).
    ...
    try {
      ...
      if (req.method === 'GET' && p === '/profiles') {
        ...
        const rows = await Promise.all(identityHomes(rootOf(here)).map(async ({ name, dir }) => {
          ...
          const origin = port
            ? (rec.httpsPort ? secureOrigin(handle, rec.httpsPort) : namedOrigin(handle, port))
            : null;
        ...
        return json(res, 200, {
          ...
          origins: {
            loopback: publicOrigin
              || (agent.httpsPort ? `https://localhost:${agent.httpsPort}/` : `http://localhost:${port}/`),
            named: publicOrigin
              ? null
              : allowed.label
                ? (agent.httpsPort
                  ? `https://${allowed.label}.localhost:${agent.httpsPort}/`
        ...
        case '/new-actor': {
          ...
          if (started.status !== 202) {
            return json(res, started.status || 502, {
              ...
              port: newPort, url: `${agent.httpsPort ? secureOrigin(handle, newPort + 1000) : namedOrigin(handle, newPort)}${SETUP_PAGE}`,
            });
          }
          return json(res, 200,
            { ok: true, handle, port: newPort,
              url: `${agent.httpsPort ? secureOrigin(handle, newPort + 1000) : namedOrigin(handle, newPort)}${SETUP_PAGE}` });
        }
          ...
          if (want) {
            ...
            if (live) {
              return json(res, 200, {
                ...
                url: `${live.httpsPort ? secureOrigin(live.handle || name, live.httpsPort) : namedOrigin(live.handle || name, want)}/`,
              });
            }
          }
          ...
          for (let i = 0; i < 60; i++) {
            ...
            if (up) {
              return json(res, 200, { ok: true, name, port: on,
                url: `${up.httpsPort ? secureOrigin(up.handle || name, up.httpsPort) : namedOrigin(up.handle || name, on)}/` });
            }
          }
...
export function startAdmin({ port, gateToken, agent, log = console.log, handle = null,
  ...
  const onListen = () => {
    ...
    // how two identities end up in one browser storage bucket. When https is
    // coming up its own line names the browsing origin; this one is the
    // cleartext fallback.
    log(tls && httpsPort
      ? `FediPod http on ${namedOrigin(allowed.label, port)}/`
      : `FediPod on ${namedOrigin(allowed.label, port)}/ (UI + API)`);
    // Hold the well-known door: whoever answers on 8030 sends the browser to
    // its own record page, which lists every identity on this machine. Only a
    // configured agent qualifies — an unconfigured one's record page is a
    // setup form with no identity list.
    const door = claimDirectory({ port,
      origin: () => (agent.httpsPort
        ? secureOrigin(allowed.label, agent.httpsPort)
        : namedOrigin(allowed.label, port)),
      log, eligible: () => agent.configured() });
  ...
  // certificate. https is the advertised origin; a failure here degrades to
  // http-only, never exits.
  if (tls && httpsPort) {
    ...
    httpsServer.on('listening', () => {
      log(`FediPod on ${secureOrigin(allowed.label, httpsPort)}/ (UI + API)`
