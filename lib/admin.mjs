  ...
  const handler = async (req, res) => {
    ...
    try {
      ...
      if (req.method === 'GET' && p === '/config') {
        ...
        const frontName = cfg.gateway?.frontActor?.match(/\/u\/([^/]+)\/ap\/actor\/?$/)?.[1] || null;
        const address = frontName && cfg.gateway?.frontActor
          ? `@${frontName}@${new URL(cfg.gateway.frontActor).host}`
