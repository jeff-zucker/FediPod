...
export class Publisher {
  ...
  async publishNote(content, { inReplyTo, attachments } = {}) {
    ...
    // A reply keeps the thread's mentions — the group included — even when the
    // client did not prefill them. This is what stops a group thread breaking
    // the moment somebody trims the handles out of their reply.
    const carried = inReplyTo
      ...
      : [];
    const mentions = [];
    for (const handle of [...new Set([...wire.mentionsIn(content), ...carried])]) {
      if (!this.resolveMention) break;
      const doc = await this.resolveMention(handle).catch(() => null);
      if (doc?.id) mentions.push({ handle, actor: doc.id, inbox: doc.endpoints?.sharedInbox || doc.inbox });
      else this.log(`mention @${handle} did not resolve — left as text`);
    }
