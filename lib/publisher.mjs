...
import { noteFieldsFrom } from './podrdf.mjs';
...
export class Publisher {
  ...
  async ensurePrivateAcls() {
    ...
    const findings = await containers.repairPrivateAcls(this.remote, onPod,
  ...
  async rebuildStatuses({ fromNotes = false } = {}) {
    ...
    for (const s of recovered) {
      try {
        // From the status record, not a wire document — a recovered post has
        // only what statuses.json kept, so its addressing is not reconstructed.
        // Better an absent field than a guessed one.
  ...
  async publishQuestion(content, { options = [], multiple = false, expiresAt = null,
    ...
    await this.local.writeNote('posts', slug, {
      ...
      inReplyTo, attachments: [], ...noteFieldsFrom(question),
    });
    this.store.addStatus({
  ...
  async updateNote(s, { content, spoilerText = null, attachments = null } = {}) {
    ...
    if (s.slug) {
      await this.local.writeNote('posts', s.slug, {
        ...
        inReplyTo: s.inReplyTo, attachments: atts, ...noteFieldsFrom(note),
      });
    }
