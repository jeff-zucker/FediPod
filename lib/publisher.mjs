...
export class Publisher {
  ...
  async publishProfile() {
    ...
    await this.assertPrivateTrees();
    this.log(`profile published: @${this.config.handle}@${host} → ${urls.actor}`);
  }

  // An ACL write that silently failed would leave the state tree — private
  // keys included — world-readable. Prove otherwise with an UNauthenticated
  // read, and say so loudly if it succeeds.
  async assertPrivateTrees() {
    for (const url of [this.urls.state, this.urls.fediverse]) {
      try {
        const res = await fetch(url, { headers: { accept: 'text/turtle' } });
        if (res.status < 400) {
          this.log(`SECURITY: ${url} is readable without credentials — check the pod's ACLs`);
        }
      } catch { /* unreachable is not a finding here */ }
    }
  }
