...
export class Publisher {
  ...
  async publishOutbox(outbox, { acls = false } = {}) {
    const { urls } = this;
    const pages = wire.outboxPages(outbox);
    const seen = this.store.read('published.json', {});
    const before = seen.outboxPages || {};
    const after = {};
    let wrote = 0;
    ...
    await this.remote.putJson(urls.outbox, wire.outboxHead(urls.outbox, outbox.length));
    if (acls) await this.remote.setAcl(urls.outbox, ['Read']);
    this.store.write('published.json',
      { ...this.store.read('published.json', {}), outboxPages: after });
    return wrote;
  }
  ...
  async publishCollections(which = ALL_COLLECTIONS) {
    ...
    if (which.outbox) {
      ...
      await this.publishOutbox(outbox, { acls: which.acls });
    }
