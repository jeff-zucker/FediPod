...
export class Streaming {
  constructor({ masto, log = console.log, allowed = null, gate = null }) {
    this.masto = masto;
    this.log = log;
  ...
  attach(server) {
    server.on('upgrade', (req, socket) => {
      ...
      if (bad) {
        this.log(`streaming refused: ${bad}`);
        socket.write('HTTP/1.1 403 Forbidden\r\n\r\n');
        socket.destroy();
        return;
      }
      // AP_GATE_TOKEN covers every other route; an upgrade that skipped it was
      // a hole in a control the operator had switched on.
      if (this.gate && !this.gate.upgradeOk(req)) {
        this.log('streaming refused: missing or bad gate token');
