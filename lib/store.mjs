...
export class PodStore {
  ...
  attach(base, fetchImpl) {
    ...
    // ETags across would serve one pod's documents as another's.
    if (this.base && this.base !== next) { this.cache.clear(); this.etags.clear(); }
    this.base = next;
    this.fetchImpl = fetchImpl;
  }
      ...
      for (let attempt = 1; attempt <= PUT_RETRIES; attempt++) {
        try {
          ...
          if (res.status < 500 && res.status !== 429) {
            this.log(`state PUT ${name} refused (HTTP ${res.status}) — not retrying`);
            return false;
          }
          ...
          if (attempt === PUT_RETRIES) { this.log(`state PUT ${name} gave up: ${e.message}`); return false; }
          const ladder = Math.min(attempt * 2000, 30_000);
          const wait = e.retryAfterMs || Math.round(ladder * (0.8 + Math.random() * 0.4));
  ...
  // of handling it is written down. With no fetchImpl the store is pure memory
