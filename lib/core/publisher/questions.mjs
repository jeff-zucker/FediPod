...
export async function publishQuestion(publisher, content, { options = [], multiple = false, expiresAt = null,
  ...
  const question = wire.questionDoc({
    ...
    visibility, summary: spoilerText, sensitive, also: reply ? [reply.actor] : [],
...
export async function republishPoll(publisher, questionId, { closing = null } = {}) {
  ...
  const question = wire.questionDoc({
    ...
    attachments: [], mentions, also: s.replyActor ? [s.replyActor] : [],
    visibility: s.visibility || 'public', summary: s.spoiler || null, sensitive: !!s.sensitive, container,
