...
export async function readPublishedOutbox(publisher) {
  const items = await podOutbox.readPublished(publisher.remote, publisher.urls);
  return Array.isArray(items) ? items.map(wire.outboxLocalItem) : items;
}
...
// included. A post is recorded by its note id and published as its Create
// (wire.outboxWireItem); an Announce has only a fragment id, so the activity
// itself goes in.
