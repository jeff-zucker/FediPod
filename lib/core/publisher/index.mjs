...
export class Publisher {
  ...
  async publishProfilePage({ force = false } = {}) {
    const { urls } = this;
    const host = wire.publicHandle(this.config).split('@')[1] || new URL(urls.base).host;
    ...
    if (!force && seen.pageDigest === digest) { this.log('profile page unchanged'); return false; }
  ...
  async publishProfile({ force = false } = {}) {
    ...
    const actorDoc = this.actorDocFor({ priv, moderators, inbox: gwActive ? gw.url : null });
