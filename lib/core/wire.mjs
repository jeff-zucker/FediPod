...
// does not.
// The legacy container name, kept only as the fallback for accounts made
// before setup pinned a root. New accounts on every build — BrowserAgent,
// DeviceAgent and Server — write their data into a `fedipod/` container in the
// pod. On a Server the operator door is served at `/fp/`, off that container.
