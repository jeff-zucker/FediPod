...
// does not.
// The legacy container name, kept as the fallback for accounts made before
// setup pinned a root, and still the default for a Server account (whose
// operator door is served at `/fedipod/` on the pod's own origin, so its AP
// tree cannot also be `/fedipod/`). New DeviceAgent and BrowserAgent accounts
// use `fedipod/`, chosen at setup and written into the credential.
