...
export function jrd({ handle, host, actor, aliases = [], page = null }) {
  return {
...
export { profilePageHtml } from './profile-page.mjs';
...
// The outbox lists activities. A post is recorded by its object id, which is
// what every local reader keys on, and published as the Create beside it — a
// document of its own. An Announce is recorded and published whole.
export const outboxWireItem = (i) => (typeof i === 'string' ? createActivityId(i) : i);
export const outboxLocalItem = (i) =>
  (typeof i === 'string' && i.endsWith('-create') ? i.slice(0, -'-create'.length) : i);
...
// Packed with its parser into one file (scripts/vendor-sanitize.mjs): the
// parser ships only as an ES module, which the Node running the Netlify
// functions will not require() from sanitize-html.
import sanitize from '../../vendor/sanitize-html.cjs';
...
// #word in the text: letters, digits and underscores with at least one
// letter, not glued to a word or a path (so a URL's fragment is not a tag).
export const HASHTAG_RE = /(^|[^\/\w)])#([\p{L}\p{N}_]*[\p{L}_][\p{L}\p{N}_]*)/gu;
...
// Where a tag's link points: this actor's own page, asked for the tag. A
// Hashtag's href must be a URL; the name is what servers file it under.
export function tagHref(urls, name) {
...
export function contentHtml(text, mentions = [], hashtags = []) {
...
// The quote terms, declared exactly as Mastodon declares them on a post:
// FEP-044f's `quote` and `quoteAuthorization`, and the two older spellings
// (Fedibird's quoteUri, Misskey's _misskey_quote) that older servers read.
export const QUOTE_CTX = {
...
// The interaction policy terms, GoToSocial's, declared as Mastodon declares them.
export const POLICY_CTX = {
...
// Who may quote a post: anyone, for a post the world can read; nobody, for a
// followers-only or direct one. Said on every note, because a server that
// finds no policy offers no quote button.
export function quotePolicy(visibility) {
  const open = visibility === 'public' || visibility === 'unlisted';
  return { canQuote: { automaticApproval: open ? [PUBLIC] : [], manualApproval: [] } };
}

// `also`: actors addressed without being mentioned — the parent's author on
// a reply. In cc but not in tag, so their server receives the post and does
// not notify them. A direct post names its audience in the text alone.
// `quote`: the post this one quotes, `{ id, authorization }` — the
// authorization is the quoted author's QuoteAuthorization, once granted.
export function noteDoc({ urls, slug, content, published, inReplyTo, attachments, mentions = [],
  visibility = 'public', summary = null, sensitive = false, updated = null, container = null, also = [],
  quote = null }) {
  ...
  if (quote?.id) {
    // All three spellings name the same post, as Mastodon writes them.
    note.quote = quote.id;
  ...
  // A content warning is what a receiver hides behind; media marked
  // sensitive without one is hidden the same way.
