...
// A 410 says the inbox is gone for good and a 404 that nothing answers there:
// neither earns a second POST. Gone also means whoever read there has left.
const unsalvageable = (e) => e.status === 404 || e.status === 410;
...
export class Deliverer {
  ...
  constructor({ store, keyId, rsaPrivate, actorId = null, edPrivate = null, proofKeyId = null,
    log = console.log, passive = false, onGone = null }) {
    this.store = store;
    // Called after followers at a gone inbox are dropped: the agent republishes
    // its followers collection.
    this.onGone = onGone;
    this.keyId = keyId;
  ...
  async _unsalvageable(inbox, e) {
    if (e.status !== 410) { this.log(`delivery dropped (${e.message}) — nothing answers there`); return; }
    ...
    try { await this.onGone?.(); } catch (err) { this.log(`followers republish after 410: ${err.message}`); }
  }
  ...
  async _drainQueueOnce() {
    ...
    for (const item of q) {
      ...
      try {
        ...
        if (unsalvageable(e)) { changed = true; await this._unsalvageable(item.inbox, e); continue; }
        item.attempts += 1;
        changed = true;
