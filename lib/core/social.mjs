...
export async function favourite(agent, s, { serial = Date.now() } = {}) {
  ...
  const act = wire.likeActivity({ urls: agent.publisher.urls, noteId: s.noteId, serial });
...
export async function deleteNote(agent, s) {
  ...
  const inboxes = agent.store.getContacts().followers
    .map(f => f.sharedInbox || f.inbox).filter(Boolean);
  const del = wire.deleteActivity({ urls, noteId: s.noteId });
...
export async function pinStatus(agent, s, pinned) {
  ...
  if (agent.publisher.publishProfilePage) {
    await agent.publisher.publishProfilePage().catch(e => agent.log?.(`profile page: ${e.message}`));
  }
