...
export class Lease {
  ...
  // passes one it keeps.
  constructor({ url, fetchImpl, log = console.log, id = null }) {
    this.url = url;
    ...
    this.unreadable = null;   // and, when unreadable, what the read got: "HTTP 403", or the error
  }
  ...
  async readFresh() {
    ...
    try {
      ...
      if (res.status >= 400) { this.unreadable = `HTTP ${res.status}`; return UNREADABLE; }
      body = await res.text();
    } catch (e) { this.unreadable = e.message; return UNREADABLE; }
  ...
  async acquire() {
    ...
    if (cur === UNREADABLE) {
      ...
      this.log(`lease unreadable (${this.unreadable}) — staying a viewer`); return false;
    }
