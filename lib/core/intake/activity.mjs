...
export function trimActivity(a) {
  ...
  if (obj) {
    // An Update says what to change INSIDE its object — a topic's new name, or
    // whether it is closed — so flattening the object to its id alone threw the
    // instruction away and left a bare topic id that meant nothing. Close,
    // reopen and rename all arrived at the queue saying only which topic, and
    // the moderator watched them do nothing. Kept now, bounded, and only the
    // two fields a moderation queue acts on: everything else is still dropped.
    const asked = a.object && typeof a.object === 'object' ? a.object : null;
    const said = {};
    if (asked) {
      if (typeof asked.name === 'string') said.name = asked.name.slice(0, 300);
      // `false` is the whole of a reopen, so presence is what counts, not truth.
      if ('closed' in asked) {
        // A date stays a date; a boolean stays a boolean; and the STRINGS
        // "true"/"false" — which is what a boolean looks like after a form, a
        // template or an RDF round trip — become the booleans they mean,
        // because `!!"false"` is true and would turn a reopen into a close.
        const v = asked.closed;
        said.closed = v === 'false' ? false : v === 'true' ? true
          : typeof v === 'string' ? v.slice(0, 64) : !!v;
      }
    }
    out.object = Object.keys(said).length
      ? { id: String(obj).slice(0, 2048), ...said }
      : String(obj).slice(0, 2048);
  }
...
// A reaction: an emoji somebody put on a post. Pleroma, Akkoma, Misskey and
// Sharkey send EmojiReact; Misskey also sends a Like whose `content` is the
// emoji, which is what its own users see as a reaction. Either way the
// emoji is the content — a unicode character, or `:name:` for a custom one
// whose image rides the tag list. Null when the activity is a plain Like.
export const MAX_REACTION_CHARS = 64;
export function reactionOf(activity) {
  if (!activity || (activity.type !== 'EmojiReact' && activity.type !== 'Like')) return null;
  const content = typeof activity.content === 'string' ? activity.content.trim() : '';
  if (!content) return null;
  const emoji = content.slice(0, MAX_REACTION_CHARS);
  const bare = emoji.replace(/^:|:$/g, '');
  const custom = emojisOf(activity).find(e => e.shortcode === bare);
  return { emoji, ...(custom ? { url: custom.url } : {}) };
}

// What a note quotes, whichever way its server says it. Mastodon writes
// FEP-044f's `quote` alongside the two older spellings, Fedibird's quoteUri
// and Misskey's _misskey_quote; Pleroma writes quoteUrl; FEP-e232 puts a Link
// in the tag list with the Misskey relation. One post, one quote.
const QUOTE_REL = 'https://misskey-hub.net/ns#_misskey_quote';
export function quoteOf(note) {
  const idOf = (v) => (typeof v === 'string' ? v : v?.id);
  const named = [note?.quote, note?.quoteUri, note?._misskey_quote, note?.quoteUrl]
    .map(v => idOf(Array.isArray(v) ? v[0] : v)).find(Boolean);
  const linked = [].concat(note?.tag || []).find(t => t?.type === 'Link' && t.href
    && [].concat(t.rel || []).includes(QUOTE_REL))?.href;
  const q = named || linked;
  return q && q !== note?.id ? httpOnly(String(q).slice(0, MAX_URL_CHARS)) : null;
}

// Who the author lets quote a post: their interaction policy's canQuote, in
// the words a client uses. The public collection is `public`; the author's
// own followers or following collections are `followers` and `following`.
// Null when the note says nothing, which is the common case on servers that
// predate the policy.
export function quotePolicyOf(note, author) {
  const can = note?.interactionPolicy?.canQuote;
  if (!can || typeof can !== 'object') return null;
  const word = (v) => {
    const id = typeof v === 'string' ? v : v?.id;
    if (!id) return null;
    if (id === 'https://www.w3.org/ns/activitystreams#Public' || id === 'as:Public' || id === 'Public') return 'public';
    if (author && sameIdentity(id, author)) {
      if (/\/followers\/?$/u.test(id)) return 'followers';
      if (/\/following\/?$/u.test(id)) return 'following';
    }
    return null;
  };
  const list = (v) => [...new Set([].concat(v || []).map(word).filter(Boolean))];
  return { automatic: list(can.automaticApproval), manual: list(can.manualApproval) };
}
