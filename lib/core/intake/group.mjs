...
export function isModerationAsk(intake, activity) {
  // What a forum holds beyond a group's own asks: a topic removed or moved,
  // a post deleted out of a topic, a report.
...
export async function amplify(intake, noteId, { approved = false, activity = null } = {}) {
  ...
  //
  // Except a PRIVATE group, where that reasoning runs backwards. Its members
  // write to the group and to nobody else on purpose, and its membership is
  // exactly the people already allowed to read what they write; refusing the
  // carry there would mean a private group could only carry posts its author
  // had published to the world. So a group told it is private carries what is
  // addressed to it. A DM is still a DM and is still never carried.
  ...
  if (!contacts.followers.some(f => f.actor === s.actor)) {
    // A group carries its members and nobody else. A forum may choose to
    // HOLD a stranger's reply for a moderator instead of dropping it, which
    // is what `replyPolicy` says; the group itself still carries nothing it
    // has not been told to.
    if (intake.onStranger) {
      const held = await intake.onStranger({ noteId, actor: s.actor, activity }).catch(() => false);
      if (held) { intake.log(`held for a moderator — ${s.actor} is not a member`); return; }
    }
    // The hook may have made them one — a forum whose categories are open
    // treats posting into one as joining it. So ask the record again rather
    // than the copy taken before the hook ran.
  ...
  // places the post in its topic here — after the carry, so only what the
  // category vouched for is in the topic (FEP-7888 ownership). Its failure
  // never undoes the carry.
  if (intake.onCarried) {
    // Both: what the group said (the Announce) and what its author sent. A
    // forum reads the author's own activity — the name they gave the topic
    // they were opening is on that one, and on no other.
    await intake.onCarried({ noteId, actor: s.actor, activity: act, sent: activity })
      .catch(e => intake.log(`after the carry of ${noteId}: ${e.message}`));
  }
