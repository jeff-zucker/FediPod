...
import { httpOnly, emojisOf, pollOf, isContentType, authorOf, quoteOf, quotePolicyOf, MAX_MENTIONS, MAX_URL_CHARS, MAX_REPLIES_RECORDED, MAX_FORWARDED, FORWARDABLE, FORWARD_TYPES } from './activity.mjs';
...
export function concernsUs(intake, doc, actor) {
  ...
  // A post that quotes one of ours is about us as much as a reply is.
  const quoted = quoteOf(doc);
  if (quoted && quoted.startsWith(intake.urls.notes)) return true;
...
export async function ingestNote(intake, objectId, actor, { via, inline = null } = {}) {
  ...
  // The verified document, kept for whoever acts on this note after the
  // carry: a forum caches it for its readers. Only when someone asked.
  ...
  const mentions = [].concat(note.tag || [])
    ...
    .filter(m => m.href);
  const hashtags = [].concat(note.tag || [])
    .filter(t => t?.type === 'Hashtag' && t.name)
    .slice(0, MAX_MENTIONS)
    .map(t => ({ name: String(t.name).replace(/^#/, '').slice(0, 128), url: httpOnly(String(t.href || '').slice(0, MAX_URL_CHARS)) }))
    .filter(t => t.name);
  const emojis = emojisOf(note);
  const poll = pollOf(note);
  // What the note quotes, and what its author lets others quote. The quoted
  // post is fetched and kept so the quote can be shown whole; whether the
  // quote was allowed is checked, never assumed.
  const quote = quoteOf(note);
  const quotePolicy = quotePolicyOf(note, author);
  const quoteState = quote ? await intake.quoteStateOf(note, quote) : null;
  if (quote) await holdQuoted(intake, quote);
  ...
  // raise nothing and sat unseen in the private mentions. A post quoting
  // one of ours is its own kind of notification.
  ...
  const quotesUs = !!quote && quote.startsWith(intake.urls.notes);
  if (quotesUs) {
    intake.store.addNotification({ type: 'quote', actor: author, noteId: note.id });
  } else if (!followed || replyToOurs || direct || namesUs) {
...
// The post a quote shows: fetched at its origin and kept out of every
// timeline (`remote`), the way a thread's ancestors are — it is there to be
// shown inside the quoting post, not on its own. Nothing is fetched twice,
// and nothing is followed further: a quote of a quote shows one level.
export async function holdQuoted(intake, quoteId) {
  if (intake.store.getStatuses().some(s => s.noteId === quoteId)) return;
  if (intake.store.isBlocked(quoteId)) return;
  const q = await intake.fetchAP(quoteId).catch(() => null);
  if (!q || q.id !== quoteId || !isContentType(q.type)) {
    intake.log(`quoted post ${quoteId} could not be read — the quote shows without it`);
    return;
  }
  const by = authorOf(q, null);
  if (!by || intake.store.isBlocked(by)) return;
  const { attachmentsOf, titledContent } = await import('../wire.mjs');
  const attachments = attachmentsOf(q);
  const emojis = emojisOf(q);
  const poll = pollOf(q);
  const quotePolicy = quotePolicyOf(q, by);
  const mentions = [].concat(q.tag || [])
    .filter(t => t?.type === 'Mention' && t.href && t.name)
    .slice(0, MAX_MENTIONS)
    .map(t => ({ href: httpOnly(String(t.href).slice(0, MAX_URL_CHARS)), name: String(t.name).slice(0, 256) }))
    .filter(m => m.href);
  intake.store.addStatus({
    noteId: q.id, actor: by, content: titledContent(q),
    published: q.published, inReplyTo: q.inReplyTo, kind: 'remote',
    ...(q.summary ? { spoiler: String(q.summary).replace(/<[^>]*>/g, '') } : {}),
  ...
  intake.log(`quoted post kept: ${q.id}`);
}
