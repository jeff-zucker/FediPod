...
import { trimActivity, sameIdentity, sameOrigin, httpUrl, reactionOf, MAX_EMOJIS, MAX_ITEM_BYTES, MAX_FORWARDS_PER_DRAIN } from './activity.mjs';
...
export class Intake {
  ...
  async _drainOnce() {
    ...
    // What a drain leaves behind may need acting on: a forum's queue of asks,
    // which is filled here and emptied by whoever can vouch for them.
  ...
  async handle(activity, receipt = null) {   // eslint-disable-line no-unused-vars
    ...
    switch (activity.type) {
      ...
      case 'QuoteRequest': return this.onQuoteRequest(activity, actor, { trusted });
      case 'Like': case 'Announce': case 'EmojiReact': {
        ...
        // An emoji on one of our posts. Recorded the way a Like is — on the
        // sender's word, nothing to dereference — with the emoji itself, and
        // a custom emoji's image added to the post's own list so a client
        // that draws reactions from there finds it. Anything else an
        // EmojiReact names is not ours to notice.
        const reaction = activity.type !== 'Announce' ? reactionOf(activity) : null;
        if (reaction) {
          if (!objectId || !objectId.startsWith(this.urls.notes)) return;
          this.store.addNotification({
            type: 'reaction', actor, noteId: objectId, emoji: reaction.emoji,
            ...(reaction.url ? { url: reaction.url } : {}),
          ...
          if (reaction.url) {
            ...
            if (s && !(s.emojis || []).some(e => e.shortcode === shortcode) && (s.emojis || []).length < MAX_EMOJIS) {
              this.store.updateStatus(objectId, { emojis: [...(s.emojis || []), { shortcode, url: reaction.url }] });
            }
          }
          return;
        }
        if (activity.type === 'EmojiReact') return;
      ...
      case 'Block': return this.onBlock(activity, actor, { trusted });
      case 'Flag': if (this.onReport) return this.onReport(activity, actor, { trusted }); return;
  ...
  onBlock(...a) { return activities.onBlock(this, ...a); }
  ...
  onQuoteRequest(...a) { return activities.onQuoteRequest(this, ...a); }
  quoteStateOf(...a) { return activities.quoteStateOf(this, ...a); }
