 ...
 */
/**
 * What a `null` in the document meant, carried onto the reading of it.
 *
 * ActivityPub §6.3.1: in an Update, a key set to `null` says REMOVE that
 * property. JSON-LD has no way to carry that — expansion drops a null outright
 * — so a partial update that takes something away arrives through the graph
 * saying nothing at all, and is indistinguishable from an Update that never
 * mentioned the property. The document plainly said it, so it is put back.
 *
 * Only nulls, only on the activity and its object, and only where the reading
 * does not already have the key: nothing here overrides what the graph carried.
 */
function carryRemovals(input, view) {
  ...
  for (const [key, value] of Object.entries(input)) {
    ...
    } else if (key === 'object' && typeof value === 'object' && !Array.isArray(value)
      && view.object && typeof view.object === 'object' && !Array.isArray(view.object)) {
...
export async function readLenient(raw) {
  ...
  try {
    ...
    return { doc, graph, view: carryRemovals(input, graphView(graph, { root })), degraded: null };
  } catch (e) {
    ...
    try {
      ...
      return { doc: input, graph, view: carryRemovals(input, graphView(graph, { root })), degraded: e.message };
    } catch (inner) {
