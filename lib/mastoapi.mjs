...
export class MastoApi {
  ...
  async handle(req, res, pathname, url) {
    ...
    if (pathname === '/api/v1/instance') {
      return send(200, {
        uri: this.host, title: 'activitypod-js', short_description: 'Solid pod ActivityPub actor',
        description: '', email: '', version: '4.2.0 (compatible; activitypod-js)',
    ...
    if (pathname === '/api/v2/instance') {
      return send(200, {
        domain: this.host, title: 'activitypod-js', version: '4.2.0 (compatible; activitypod-js)',
        source_url: 'https://github.com/jeff-zucker/activitypod-js', description: 'Solid pod ActivityPub actor',
    ...
    // A viewer-mode agent (another agent holds the drain lease) must not
    // mutate shared state or federate — reads only.
    if (this.agent.viewer && req.method !== 'GET' && req.method !== 'HEAD') {
      return send(503, { error: 'another agent is active for this pod — this one is read-only' });
    }
    ...
    if (pathname === '/api/v1/push/subscription') {
      return send(404, { error: 'Record not found' });   // no web-push — clients poll or stream
    }
...
<title>activitypod-js — log in</title>
<style>body{font:16px system-ui,sans-serif;max-width:22rem;margin:15vh auto;padding:0 1rem}
input,button{font:inherit;width:100%;padding:.5rem;margin:.3rem 0;box-sizing:border-box}
.err{color:#b00020}</style></head><body>
<h1>activitypod-js</h1>
