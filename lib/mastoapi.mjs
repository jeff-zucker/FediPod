...
export class MastoApi {
  ...
  async handle(req, res, pathname, url) {
    ...
    if (pathname === '/api/v1/statuses' && req.method === 'POST') {
      ...
      // The pod publishes every note document world-readable, so the two
      // visibilities that promise less than that are refused rather than
      // silently published public.
      const visibility = body.visibility || 'public';
      if (visibility !== 'public' && visibility !== 'unlisted') {
        return send(422, { error: 'followers-only and direct posts are not supported — the pod stores posts world-readable' });
      }
    ...
    // Pinned is this profile's own arrangement; it shows on our pages and in
    // our API, and is not published to other servers.
