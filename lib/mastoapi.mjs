...
export class MastoApi {
  ...
  account(actorUrl, { selfAcct } = {}) {
    ...
    return {
      ...
      following_count: self ? this.store.getContacts().following.length : (cached.counts?.following ?? 0),
  ...
  page(items, url, { limit = 20, max = 40 } = {}) {
    const n = Math.min(Number(url.searchParams.get('limit')) || limit, max);
    const idOf = (s) => this.store.idFor(s.noteId);
    const cut = (param, keepAfter) => {
      const v = url.searchParams.get(param);
      if (!v) return null;
      const i = items.findIndex(s => idOf(s) === v);
      return i < 0 ? null : i;
    };
  ...
  async handle(req, res, pathname, url) {
    ...
    if (pathname === '/oauth/authorize' && (req.method === 'GET' || req.method === 'POST')) {
      ...
      if (req.method === 'POST') {
      ...
      } else if (this.allowed && !this.allowed.isLocal(req.headers.host)) {
        ...
        return send(403, {
          error: 'this agent answers on an address outside this machine and has no password set — '
            + 'run `activitypod passwd` before logging in over that address',
        });
      }
    ...
    if (mAccStatuses) {
      ...
      const all = this.store.getStatuses();
      const { items, headers } = this.page(all.filter(s => s.actor === actorUrl), url);
      return send(200, items.map(s => this.status(s, { all })), headers);
    }
