...
export class MastoApi {
  constructor({ agent, log = console.log, allowed = null, scheme = null }) {
    this.agent = agent;
    this.log = log;
  ...
  async handle(req, res, pathname, url) {
    ...
    if (pathname === '/oauth/authorize' && (req.method === 'GET' || req.method === 'POST')) {
      ...
      // A registered client redirecting to its OWN origin — or to no origin
      // at all (the out-of-band urn, pasted by hand) — is the third-party
      // flow: the code is bound to it and exchanged with its secret. A
      // redirect back to this agent is the local flow, unchanged.
      const external = !!app
        && (redirect === 'urn:ietf:wg:oauth:2.0:oob' || !this.redirectAllowed(redirect));
