...
export async function handle(api, ctx) {
  ...
  if (pathname === '/api/v1/statuses' && req.method === 'POST') {
    ...
    // A quote: the post named is one this account holds, and its author has
    // said it may be quoted. The author's answer decides what the reader sees;
    // the reasons a quote cannot even be asked for are the client's to show.
    let quote = null;
    if (body.quoted_status_id) {
      const quotedUrl = api.store.urlFor(String(body.quoted_status_id));
      const quoted = quotedUrl && api.store.getStatuses().find(x => x.noteId === quotedUrl);
      if (!quoted) return send(404, { error: 'Record not found' });
      if (['bsky', 'acct'].includes(quoted.kind)) return send(422, { error: 'that post cannot be quoted from this account' });
      if (visibility === 'private' || visibility === 'direct') return send(422, { error: 'a quote post is public or unlisted' });
      const may = api.quoteApproval(quoted).current_user;
      if (may === 'denied') return send(422, { error: 'the author of that post does not allow quotes' });
      if (may === 'unknown') return send(422, { error: 'the author of that post has not said whether it may be quoted' });
      if (body.scheduled_at) return send(422, { error: 'a quote cannot be scheduled' });
      quote = { id: quoted.noteId, actor: quoted.actor };
    }
    ...
    if (asking) {
      ...
      if (quote) return send(422, { error: 'a poll cannot quote a post' });
      ...
      try {
        const q = await api.agent.publisher.publishQuestion(body.status, {
          ...
          inReplyTo, visibility, spoilerText, sensitive,
        });
    ...
    try {
      note = await api.agent.publisher.publishNote(body.status,
        { inReplyTo, attachments, visibility, spoilerText, sensitive, quote });
    } catch (e) {
  ...
  // Who may quote a post is the same for every post here — anyone, when the
  // world can read it — and is not set per post.
  if (/^\/api\/v1\/statuses\/[a-f0-9]+\/interaction_policy$/.test(pathname) && req.method === 'PUT') {
    return send(422, { error: 'the quote policy is the same for every post here: anyone may quote a public or unlisted post, nobody a followers-only or direct one' });
  }
  ...
  if (mHistory && req.method === 'GET') {
    ...
    if (!s) return send(404, { error: 'Record not found' });
    return send(200, [{
      content: s.content || '', spoiler_text: s.spoiler || '', sensitive: !!(s.spoiler || s.sensitive),
  ...
  if (mStatus && req.method === 'PUT') {
    ...
    const spoilerText = String(body.spoiler_text || '').trim() || null;
    const sensitive = body.sensitive === undefined ? null
      : (body.sensitive === true || body.sensitive === 'true' || body.sensitive === '1');
