...
// 1x1 transparent PNG — placeholder header, and the instance thumbnail. A
// banner nobody set should take up no room and show nothing; an avatar is a
...
// The avatar for an account with no icon of its own: a head and shoulders on a
// slate square, 64x64. It used to be the transparent pixel above, which is
// correct JSON and invisible on screen — in Sengi's left bar the account button
// (the one control that opens the account) rendered as an empty square, so
// clicking it looked like nothing happening.
//
// A PNG rather than the SVG this was first written as, and not by preference:
// Angular sanitizes every `<img src>` binding and trusts raster data URIs only,
// because an SVG can carry script. An `svg+xml` one comes back out of it as
// `unsafe:data:image/svg+xml` and draws nothing. Sengi is Angular.
//
// Redraw it with `node claude/scratch/make-default-avatar.mjs`.
export const DEFAULT_AVATAR = 'data:image/png;base64,';
...
export function account(api, actorUrl, { selfAcct } = {}) {
  ...
  return {
    ...
    created_at: (self && api.store.getConfig()?.createdAt) || '2026-01-01T00:00:00.000Z',
    note: (self ? api.store.getConfig()?.summary : cached.summary) || '',
    url: actorUrl, uri: actorUrl,
    avatar: selfIcon(api, self, cached) || DEFAULT_AVATAR,
    avatar_static: selfIcon(api, self, cached) || DEFAULT_AVATAR,
...
export function page(api, items, url, { limit = 20, max = 40, idOf = (s) => api.store.idFor(s.noteId) } = {}) {
  ...
  // `url` is the request as the client addressed it (index.mjs handle), so
  // the links name that origin and mount, not the pod's host.
...
// The post a status quotes, and how the quote stands: `pending` until the
// quoted author allows it, `accepted` once they have, `rejected` if they
// refused. The quoted post is shown whenever it is held — a reader wants to
// see what was quoted, whatever the state says. One level deep: a quote
// inside a quote is named by id, not rendered, so the shape stays finite.
export function quoteJson(api, s, { all, depth = 0 } = {}) {
  if (!s.quote) return null;
  const state = s.quoteState || 'pending';
  const quoted = state !== 'rejected' && (all || api.store.getStatuses()).find(x => x.noteId === s.quote);
  if (depth > 0) return { state, quoted_status_id: quoted ? api.store.idFor(quoted.noteId) : null };
  return { state, quoted_status: quoted ? api.status(quoted, { all, depth: depth + 1 }) : null };
}

// Who may quote a post, in the client's words, and what that means for this
// account. Our own posts: anyone, when the world can read it; nobody
// otherwise. Another's: whatever policy the post carried — `unknown` when it
// carried none, which is what a client hides the quote button on.
export function quoteApproval(api, s) {
  if (s.actor === api.urls?.actor) {
    const open = !s.visibility || s.visibility === 'public' || s.visibility === 'unlisted';
    return { automatic: open ? ['public'] : [], manual: [], current_user: open ? 'automatic' : 'denied' };
  }
  const p = s.quotePolicy;
  if (!p) return { automatic: [], manual: [], current_user: 'unknown' };
  const follows = () => api.store.getContacts().following.some(f => f.actor === s.actor && f.accepted);
  const may = (list) => list.includes('public') || (list.includes('followers') && follows());
  const current = may(p.automatic) ? 'automatic' : may(p.manual) ? 'manual' : 'denied';
  return { automatic: p.automatic, manual: p.manual, current_user: current };
}

export function status(api, s, { all, depth = 0 } = {}) {
  ...
  return {
    ...
    sensitive: !!(s.spoiler || s.sensitive), spoiler_text: s.spoiler || '',
    ...
    quote: quoteJson(api, s, { all, depth }),
    quote_approval: quoteApproval(api, s),
...
export function pushNotify(api, n) {
  ...
  const verbs = {
    ...
    reaction: `reacted ${n.emoji || ''} to your post`, quote: 'quoted your post',
  };
...
// Mastodon's spellings for what is stored here. A reaction is what Pleroma's
// API calls an emoji reaction, and what clients render as one.
export function notificationType(api, t) {
  return t === 'follow-request' ? 'follow_request' : t === 'reaction' ? 'emoji_reaction' : t;
}
