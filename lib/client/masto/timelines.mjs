...
export async function handle(api, ctx) {
  ...
  // --- trending tags: counted off the notes already mirrored ---
  //
  // A single-actor instance has no firehose to measure, so "trending" here can
  // only mean what the tag feed has actually brought in — which is the honest
  // answer, and the one that makes a client's Explore surface show something
  // instead of an empty list. `/api/v1/trends` is Mastodon's older name for the
  // same thing.
  if (pathname === '/api/v1/trends/tags' || pathname === '/api/v1/trends') {
    const DAY = 86400000;
    const today = Math.floor(Date.now() / DAY) * DAY;
    // tag -> day -> { uses, accounts }. Accounts are counted distinctly, which
    // is what the number means in Mastodon: one person posting nine times is
    // nine uses by one account, and a client shows the second number.
    const seen = new Map();
    for (const st of api.store.getStatuses()) {
      if (st.kind !== 'tag' || !st.tag) continue;
      const at = Date.parse(st.published || '');
      if (!Number.isFinite(at) || at < today - 6 * DAY) continue;
      const day = Math.floor(at / DAY) * DAY;
      if (!seen.has(st.tag)) seen.set(st.tag, new Map());
      const days = seen.get(st.tag);
      if (!days.has(day)) days.set(day, { uses: 0, accounts: new Set() });
      const rec = days.get(day);
      rec.uses++;
      if (st.actor) rec.accounts.add(st.actor);
    }
    const followed = new Set(api.agent.tagfeed?.config().tags || []);
    const ranked = [...seen.entries()].map(([name, days]) => {
      // Seven entries, newest first, gaps filled with zeroes — a client draws
      // the sparkline straight off this and a short array draws a short line.
      const history = [];
      for (let i = 0; i < 7; i++) {
        const rec = days.get(today - i * DAY);
        history.push({ day: String(Math.floor((today - i * DAY) / 1000)),
          uses: String(rec ? rec.uses : 0), accounts: String(rec ? rec.accounts.size : 0) });
      }
      return { total: history.reduce((n, h) => n + Number(h.uses), 0),
  ...
  // The direct timeline: every direct post, newest first, paged like the rest.
  ...
  // One notification by id — what a push notification tap opens — and the
  // two ways of being rid of them.
  ...
  if (mNotif) {
    ...
    if (mNotif[2] && req.method === 'POST') { api.store.removeNotifications(x => x.id === n.id); return send(200, {}); }
    if (!mNotif[2] && req.method === 'GET') return send(200, api.notification(n));
  }

  // Grouped notifications, the shape a client reads from API version 2 on.
  // One group per notification: the client does its own gathering of
  // favourites and boosts on a post, and every field it joins on — the
  // account and status ids — is here.
  const mGroup = /^\/api\/v2\/notifications\/([A-Za-z0-9_-]+)(\/accounts|\/dismiss)?$/.exec(pathname);
  if (mGroup) {
    const n = api.store.getNotifications().find(x => x.id === mGroup[1]);
    if (!n) return send(404, { error: 'Record not found' });
    if (mGroup[2] === '/dismiss' && req.method === 'POST') { api.store.removeNotifications(x => x.id === n.id); return send(200, {}); }
    if (mGroup[2] === '/accounts' && req.method === 'GET') return send(200, [api.account(n.actor)]);
    if (!mGroup[2] && req.method === 'GET') return send(200, groupedNotifications(api, [n]));
  }
  if (pathname === '/api/v2/notifications') {
    const { page, headers } = notificationPage(api, ctx);
    return send(200, groupedNotifications(api, page), headers);
  }

  if (pathname === '/api/v1/notifications') {
    const { page, headers } = notificationPage(api, ctx);
    return send(200, page.map((n) => api.notification(n)), headers);
  }
  ...
  if (pathname === '/api/v2/search' || pathname === '/api/v1/search') {
    return searchHandler(api, ctx);
  }
  return false;
}

function groupedNotifications(api, page) {
  const accounts = new Map();
  const statuses = new Map();
  const groups = page.map((n) => {
    const acct = api.account(n.actor);
    accounts.set(acct.id, acct);
    const s = n.noteId && api.store.getStatuses().find(x => x.noteId === n.noteId);
    let statusId = null;
    if (s) { const j = api.status(s); statuses.set(j.id, j); statusId = j.id; }
    return {
      group_key: n.id, notifications_count: 1, type: api.notificationType(n.type),
      most_recent_notification_id: n.id, page_min_id: n.id, page_max_id: n.id,
      latest_page_notification_at: n.at, sample_account_ids: [acct.id],
      ...(statusId ? { status_id: statusId } : {}),
      ...(n.type === 'reaction' ? { emoji: n.emoji, ...(n.url ? { emoji_url: n.url } : {}) } : {}),
    };
  });
  return { accounts: [...accounts.values()], statuses: [...statuses.values()], notification_groups: groups };
}

// The slice of notifications a request asks for, and the Link header that
// pages it. Shared by the flat list and the grouped one.
function notificationPage(api, ctx) {
  const { req, pathname, url } = ctx;
  {
    ...
    return { page: [], headers: undefined };
  }
}

// Search: @user@host → webfinger resolve; URL → actor or note ingest;
// plain text → local mirror + actor-cache scan.
async function searchHandler(api, ctx) {
  const { req, res, pathname, url, send } = ctx;   // eslint-disable-line no-unused-vars
  {
