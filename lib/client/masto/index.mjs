...
export class MastoApi {
  ...
  async handle(req, res, pathname, url) {
    ...
    // The address the client reached, so every self-URL handed back — the
    // pagination links above all — names it rather than the pod's host.
    const scheme = String(this.scheme
      || (req.headers?.['x-forwarded-proto'] === 'https' || req.socket?.encrypted ? 'https' : 'http')).replace(/:.*$/, '');
    let self = url;
    try {
      self = new URL(`${scheme}://${req.headers?.host || 'localhost'}${this.mount || ''}${pathname}${url.search || ''}`);
    } catch { /* a Host nothing can parse: the links keep the placeholder origin */ }
    const ctx = { req, res, pathname, url: self, send };
  ...
  quoteApproval(...a) { return render.quoteApproval(this, ...a); }
