...
export async function handle(api, ctx) {
  ...
  // The instance's own directory is its one account; there is nothing to
  // suggest and nothing trending, so those screens are empty rather than 404.
  ...
  // The lists this account is on — what "add to list" from a profile reads.
  ...
  if (mAccLists && req.method === 'GET') {
    ...
    if (!actorUrl) return send(404, { error: 'Record not found' });
    return send(200, api.store.getLists().filter(l => (l.members || []).includes(actorUrl))
      .map(l => ({ id: l.id, title: l.title, replies_policy: l.repliesPolicy || 'list', exclusive: false })));
  }

  // The people muted and blocked, as accounts; the domains blocked, as names.
  ...
  if (mAccStatuses) {
    ...
    const { items, headers } = api.page(
      all.filter(s => s.actor === actorUrl && (!pinnedOnly || s.pinned))
        .filter(s => !is('exclude_replies') || !s.inReplyTo)
        .filter(s => !is('exclude_reblogs') || !isBoost(s))
        .filter(s => !is('only_media') || (s.attachments || []).length > 0)
        .filter(s => !tagged || (s.tags || []).some(t => String(t.name).toLowerCase() === tagged)), url);
