...
// can drive it. Same order, same decisions as `activitypod setup` always had:
...
// Pods that are commonly already running on this machine, for the question
// "where should the private half live?". Probed, never assumed — and a 401
// still means there IS a pod here, it just wants a token.
const LOCAL_POD_CANDIDATES = [
  { url: 'http://localhost:8000/dk-pod/', what: "data-kitchen's pod" },
  { url: 'http://localhost:4000/', what: 'a Solid server on port 4000' },
];

export async function probeLocalPods(timeoutMs = 1500) {
  const found = [];
  await Promise.all(LOCAL_POD_CANDIDATES.map(async (c) => {
    try {
      const res = await fetch(c.url, {
        headers: { accept: 'text/turtle' },
        signal: AbortSignal.timeout(timeoutMs),
      });
      if (res.status < 500) found.push({ ...c, status: res.status, gated: res.status === 401 });
    } catch { /* nothing answering there */ }
  }));
  // Declared order, not alphabetical: the first one is the one to offer, and
  // dk's is the one this agent can actually authenticate to.
  return LOCAL_POD_CANDIDATES.map(c => found.find(f => f.url === c.url)).filter(Boolean);
}
...
// (bin/activitypod.mjs, the address preview) — as data, so the page can show
...
export function preflight({ mode, pod, issuer, podName, handle, kind }) {
  ...
  if (mode === 'new') {
    // Whether a new pod gets its own subdomain is the server's call, so
    // promise nothing here we cannot keep — setup checks what it actually got.
    warnings.push('subdomain-not-guaranteed');
    return {
...
export async function runSetup({ home, agent, answers, run, deps = {}, log = () => {} }) {
  ...
  const {
    ...
    kind = 'person', approveJoins = false, summary, icon, keys, uiPassword, privateRoot,
  } = answers;
  ...
  try {
    ...
    if (resuming) {
      ...
      begin('credential');
      const credential = await mint({ origin: issuer, email, password, name: 'activitypod-js' });
      const rec = {
        ...credential,
      ...
      fs.writeFileSync(credPath, JSON.stringify(rec, null, 2) + '\n', { mode: 0o600 });
      pod = rec.remotePod;
