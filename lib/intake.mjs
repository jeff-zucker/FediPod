...
export class Intake {
  ...
  async onUndo(activity, actor) {
    ...
    // A record with no followId predates that bookkeeping and cannot be matched,
    // so it is allowed through — but ONLY for the object form. A bare IRI is by
    // construction newer than the carve-out, so it has to match.
    if (typeof activity.object === 'string' && undoneId !== rec.followId) {
      this.log(`Undo from ${actor} names ${undoneId}, which is not the follow we hold — ignored`);
      return;
    }
