...
export class Intake {
  ...
  async onCreate(activity, actor) {
    ...
    // inbound Announce is never re-announced.
    if (this.config.kind === 'group') await this.amplify(objectId);
  }
  ...
  async amplify(noteId) {
    ...
    // Drop the author's own target only when it serves nobody else: a shared
    // inbox carries the whole server's members, so skipping it would deprive
    // everyone who happens to share it with them.
    ...
    const inboxes = [...byTarget]
      .filter(([, who]) => !(who.size === 1 && who.has(s.actor)))
      .map(([t]) => t);
    const { announceActivity } = await import('./wire.mjs');
    const act = announceActivity({ urls: this.urls, noteId, serial: this.serial++ });
