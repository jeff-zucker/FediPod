    ...
    for (const item of older) {
      try {
        if (item.size > MAX_ITEM_BYTES) {
          ...
          // Bounded like every other read of an inbox item (see the drain):
          // these are documents a stranger Appended, and res.json() buffers
          // whatever they chose to send.
          const activity = res.status < 400
            ? await readCapped(res, MAX_ITEM_BYTES).then(JSON.parse).catch(() => null)
            : null;
