...
export class Intake {
  ...
  stop() { this.stopped = true; clearTimeout(this.pollTimer); this.ws?.close(); }
  ...
  async subscribe() {
    ...
    if (!body?.receiveFrom) {
      ...
      if (!this.stopped) {
        setTimeout(() => this.subscribe().catch(e => this.log(`resubscribe: ${e.message}`)), wait);
      }
      return;
    }
    ...
    this.ws.onclose = () => {
      this.wsState = 'closed';
      if (!this.stopped) {
        setTimeout(() => this.subscribe().catch(e => this.log(`resubscribe: ${e.message}`)), this._reconnectDelay());
      }
    };
