...
export class Intake {
  ...
  //
  // `keepConcerning` narrows the discard to noise: every item is read (the size
  // shortcut is off, since a large post that concerns us can't be judged unread)
  // and passed to handle(), which ingests a Create only when concernsUs passes —
  // addressed to us, a mention, a reply to ours, or from someone we follow — and
  // drops the rest.
  async prune({ before, sizeThreshold = 2048, keepConcerning = false } = {}) {
    ...
    for (const item of older) {
      try {
        if (!keepConcerning && item.size >= sizeThreshold) {
