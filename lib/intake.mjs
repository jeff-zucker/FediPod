...
// A ws:// URL is the same origin as an http:// one when host and port match and
// the scheme is the socket form of it — ws for http, wss for https. Anything
// else, including a downgrade to ws:// from an https pod, is somewhere else.
export function sameSocketOrigin(socketUrl, podBase) {
  let s, p;
  try { s = new URL(socketUrl); p = new URL(podBase); } catch { return false; }
  const want = p.protocol === 'https:' ? 'wss:' : 'ws:';
  return s.protocol === want && s.host === p.host;
}
  ...
  async onUndo(activity, actor) {
    ...
    if (!rec.followId) {
      this.log(`Undo from ${actor} cannot be matched — this follower was `
        ...
        + `so its follow id is unknown. Ignored; \`activitypod eject ${actor}\` if they did leave.`);
      return;
    }
