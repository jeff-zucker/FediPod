...
export class Intake {
  ...
  // DELETE it. When the state lives on the SAME pod as the inbox, that is safe
  // for free: a pod we cannot write to is a pod we cannot list either, so the
  // drain never starts. Once the state moves elsewhere the two can fail
  // independently, and the write has to be forced and checked first.
  // Asked per call, not cached: connect() can reattach the store.
  get strictCommit() {
    const origin = (u) => { try { return new URL(u).origin; } catch { return null; } };
    return origin(this.store?.base) !== origin(this.urls?.inbox);
  }

  async _persisted() {
    if (!this.strictCommit) return true;
  ...
  async _drainOnce() {
    ...
    for (const { url } of items) {
      ...
      try {
        ...
        // Write it down before taking it out of the mailbox. A failure here
        // leaves the item where it is: the next sweep sees it again, and a
        // re-delivered activity is already handled idempotently.
        if (!await this._persisted()) {
          this._backOff(`state not written — ${url} left in the inbox`);
          return;
        }
        if (!await this.remote.delete(url)) {
          // It is still in the mailbox. Handling is idempotent so seeing it
          // again is harmless, but counting it as handled would clear the
          // attempt record and report progress that did not happen.
        ...
        if (n >= MAX_ITEM_ATTEMPTS) {
          ...
          // written down would lose the only evidence it ever arrived.
          if (!await this._persisted()) {
            this._backOff(`state not written — ${url} left in the inbox`);
            return;
          }
  ...
  async onFollow(activity, actor) {
    ...
    // A gated group answers nothing until its operator does: no Accept, no
    // Reject. The requester's client shows "Requested", which is the standard
    // locked-account state and what manuallyApprovesFollowers told it to expect.
    if (this.config.approveJoins && !existing) {
  ...
  async onDelete(activity, actor) {
    ...
    const known = c.followers.some(f => f.actor === objectId)
      || c.following.some(f => f.actor === objectId)
      || this.store.getStatuses().some(s => s.noteId === objectId || s.actor === objectId)
      || !!this.store.getActors()[objectId];
    if (!known) return;                                   // nothing of ours to remove
