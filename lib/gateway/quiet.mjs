...
export async function routeQuietApi(request, pathname, ctx, deps) {
  ...
  if (pathname === '/api/open' && request.method === 'POST') {
    ...
    // Whom the app would name if its owner lets the gateway act for them while
    // it is closed, and whether they have (held-mail.mjs, keeper.mjs).
    return j(200, { ok: true, handle, ...await accountState(ctx, handle, rec),
      keeper: ctx.keeperWebId || null, kept: keptNow(ctx, rec), keptBy: rec.keeper?.webId || null });
  }
