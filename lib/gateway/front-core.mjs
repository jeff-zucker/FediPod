...
async function route(request, ctx) {
  ...
  // A container is Turtle; anything else here is ActivityPub. Ask for what the
  // thing can be, not for what most of them are.
  const container = podTarget.endsWith('/');
  const accept = container ? 'text/turtle, */*;q=0.8' : AP_CT;
  ...
  return { status: 200, headers: { ...open, 'content-type': container ? 'text/turtle' : AP_CT, 'cache-control': 'no-store' }, body: text };
}
