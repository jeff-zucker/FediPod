...
import { withdrawn, overLimit, relayOne } from './relay-extras.mjs';
...
const jrd = ({ handle, host, actor, aliases = [], page = null, subscribe = null }) => ({
  subject: `acct:${handle}@${host}`,
  ...(aliases.length ? { aliases } : {}),
  links: [
    { rel: 'self', type: 'application/activity+json', href: actor },
    ...(page ? [{ rel: 'http://webfinger.net/rel/profile-page', type: 'text/html', href: page }] : []),
    // Remote follow. Somebody reading this account on another server presses
    // Follow, says they are here, and that server looks for this link to find
    // out where to send them. `{uri}` is a literal the other server fills in
    // with whatever they are following — it must not be escaped.
    ...(subscribe ? [{ rel: 'http://ostatus.org/schema/1.0/subscribe', template: subscribe }] : []),
  ],
});
// The door that link names, which the site staged beside this function serves.
const subscribeTemplate = (origin) => (origin
  ? `${String(origin).replace(/\/$/u, '')}/authorize_interaction?uri={uri}` : null);
// The public profile page of any account the front answers for lives on
// its pod, whether the address is the front's or the pod's own.
const profilePageOf = (rec) => (rec?.podHome ? rec.podHome + 'ap/profile.html' : null);
...
// lives: the same origin, and — on a host that keeps pods on paths — inside
...
// this one it moved and send the Move through this one's relay. Both take a
// pod token as proof, the way the outbox door does, so any origin may ask.
const API_CORS = {
...
// A public document is the same for everybody who asks, so the edge may hold
// it and answer the next reader itself. Without this every read of an actor,
// a topic, a post or a card was a function call: one visit to a forum is
// twenty of them, and every visitor and every refresh paid again.
//
...
// it belongs to the server rather than to anybody's data, the way
// `.well-known` and `.account` do.
export const DEFAULT_RUN_PATH = '/.fediverse-account';
export const runPath = (ctx) => {
  const said = String(ctx?.runPath || '').trim().replace(/\/+$/u, '');
  if (!said) return DEFAULT_RUN_PATH;
  return said.startsWith('/') ? said : `/${said}`;
};

// A request the browser itself says came from this very origin. `Sec-Fetch-Site`
// is the browser's word and a page cannot forge it; an `Origin` that matches is
// the older way of saying the same. This is what makes it safe to read a
// session cookie as proof: a page on another site gets neither.
const sameOriginRequest = (request, origin) => {
  const site = request.headers.get('sec-fetch-site');
  if (site) return site === 'same-origin';
  const said = request.headers.get('origin');
  if (!said || !origin) return false;
  try { return new URL(said).origin === new URL(origin).origin; } catch { return false; }
};
...
function identFor(rec, policy = null) {
  return {
    // A row may name the inbox its deliveries are written to: a forum's
    // categories share one, on the forum's pod, above their own trees.
    inboxUrl: rec.inboxUrl || rec.podHome + 'ap/inbox/',
...
async function route(request, ctx) {
  ...
  // What a reader's own server speaks, asked here rather than in their
  // browser: a page cannot probe a stranger's server (no CORS), and the
  // answer decides how that reader takes part in a forum.
  if (pathname === '/api/server') {
    const host = (url.searchParams.get('host') || '').trim().toLowerCase();
    const open = { 'access-control-allow-origin': '*' };
    const said = (kind, extra = {}) => ({ ...j(200, { host, kind, ...extra }), headers: { ...j(200, {}).headers, ...open } });
    // An address at this front is a FediPod account: its client is the
    // browser here, and no other server holds it. Asked before the shape of
    // the host, since a front may answer at a name with no dot in it.
    if (host && host === String(ctx.host || '').toLowerCase()) {
      const handle = (url.searchParams.get('handle') || '').toLowerCase();
      const rec = handle ? await ctx.lookup(handle) : null;
      if (!rec) return said('fedipod');
      // Where the owner of this pod signs in: the pod says so itself, at the
      // well-known document every Solid server answers (or redirects to).
      let issuer = null;
      try {
        const res = await safeFetch(new URL('/.well-known/openid-configuration', rec.podHome).href,
          { headers: { accept: 'application/json' }, redirect: 'follow', signal: AbortSignal.timeout(SERVER_PROBE_MS) },
          ctx.fetchImpl || fetch);
        if (res.ok) issuer = JSON.parse(await readCapped(res, SERVER_PROBE_BYTES)).issuer || null;
      } catch { /* a pod that will not say leaves the reader to type it */ }
      return said('fedipod', { podHome: rec.podHome, actor: rec.actorUrl, issuer, inbox: identFor(rec).inboxUrl });
    }
    if (!/^[a-z0-9.-]+\.[a-z0-9-]+(:\d+)?$/u.test(host)) return said('invalid');
    const f = ctx.fetchImpl || fetch;
    const ask = async (path) => {
      try {
        const res = await safeFetch(`https://${host}${path}`,
          { headers: { accept: 'application/json' }, signal: AbortSignal.timeout(SERVER_PROBE_MS) }, f);
        if (!res.ok) return null;
        return JSON.parse(await readCapped(res, SERVER_PROBE_BYTES));
      } catch { return null; }
    };
    const masto = await ask('/api/v1/instance');
    if (masto) return said('mastodon-api', { name: masto.title || masto.domain || host });
    const lemmy = await ask('/api/v3/site');
    if (lemmy) return said('lemmy', { name: lemmy.site_view?.site?.name || host });
    return said('unknown');
  }
  ...
  if (pathname === '/api/attach' && request.method === 'POST') {
    ...
    // Where the door writes: the row's own tree unless the caller names an
    // inbox on the same pod — a forum's categories name the forum's.
    let inboxUrl = null;
    if (typeof body.inboxUrl === 'string' && body.inboxUrl) {
      let ok = false;
      try { ok = new URL(body.inboxUrl).origin === new URL(podHome).origin && body.inboxUrl.endsWith('/'); } catch { ok = false; }
      if (!ok) return j(400, { error: 'inboxUrl must be a container on the same pod' });
      inboxUrl = body.inboxUrl;
    }
    const record = {
      handle, address, podHome, webId: webid, actorUrl,
      kind: body.kind === 'group' ? 'group' : body.kind === 'application' ? 'application' : 'person',
  ...
  // An address at this gateway moving to another one. Its owner, proved the
  // way attach proves a pod, says where it went. From then on this front
  // answers the old actor as a moved stub — the same key under the old id,
  // `movedTo` the new one, so the Move the owner sends from the old address
  // verifies — every other old id redirects to its new address, and the
  // mail door is shut: deliveries belong at the new one. Repeating it with
  // the same target changes nothing.
  if (pathname === '/api/move' && request.method === 'OPTIONS') return apiPreflight();
  if (pathname === '/api/move' && request.method === 'POST') {
    if (!ctx.putDirectory) return withApiCors(j(501, { error: 'this front keeps no directory' }));
    let body;
    try { body = JSON.parse(await request.clone().text()); } catch { return withApiCors(j(400, { error: 'bad JSON' })); }
    const handle = String(body.handle || '').toLowerCase();
    const rec = await ctx.lookup(handle);
    if (!rec) return withApiCors(j(404, { error: 'no such account' }));
    if (rec.inboxOnly) return withApiCors(j(400, { error: 'this address lives on its pod; only an address at this gateway can move' }));
    const movedTo = String(body.movedTo || '');
    let mu;
    try { mu = new URL(movedTo); } catch { return withApiCors(j(400, { error: 'movedTo must be the new actor URL' })); }
    if (!((mu.protocol === 'https:' || (mu.protocol === 'http:' && isLoopbackHost(mu.hostname))) && /\/ap\/actor$/u.test(mu.pathname))) {
      return withApiCors(j(400, { error: 'movedTo must be an https …/ap/actor URL' }));
    }
    if (ctx.frontOrigin && mu.origin === new URL(ctx.frontOrigin).origin) return withApiCors(j(400, { error: 'that address is at this gateway' }));
    const webid = await verifyPodToken(request, pathname, ctx.verifier);
    if (!webid) return withApiCors(j(401, { error: 'a Solid-OIDC token proving the pod is required' }));
    const owner = rec.webId ? webid === rec.webId : webidUnderPod(webid, rec.podHome);
    if (!owner) return withApiCors(j(403, { error: "the token proves a different pod than this account's" }));
    const movedAt = rec.movedTo === movedTo && rec.movedAt ? rec.movedAt : new Date().toISOString();
    await ctx.putDirectory(handle, { ...rec, movedTo, movedAt });
    console.log(`front @${handle}: moved to ${movedTo}`);
  ...
  // the remote server.
  if (pathname === '/api/relay' && request.method === 'OPTIONS') return apiPreflight();
  if (pathname === '/api/relay' && request.method === 'POST') {
    let body;
    try { body = JSON.parse(await request.clone().text()); } catch { return withApiCors(j(400, { error: 'bad JSON' })); }
    const handle = String(body.handle || '').toLowerCase();
    const rec = await ctx.lookup(handle);
    if (!rec) { console.log(`relay: no account named ${handle}`); return withApiCors(j(404, { error: 'no such account' })); }
    const webid = await verifyPodToken(request, pathname, ctx.verifier);
    if (!webid) { console.log(`relay @${handle}: no usable pod token`); return withApiCors(j(401, { error: 'a Solid-OIDC token proving the pod is required' })); }
    const owner = rec.webId ? webid === rec.webId : webidUnderPod(webid, rec.podHome);
    ...
    // Something of the account's withdrawn: the edge's copies go.
    if (withdrawn(items, rec.actorUrl)) await ctx.purge?.([`u-${handle}`]).catch((e) => console.log(`purge @${handle}: ${e?.message || e}`));
    ...
    return withApiCors(j(200, { results }));
  }
  ...
  if (pathname === '/api/agent' && request.method === 'POST') {
    ...
    const ownsIt = (who) => (owners.length ? owners.includes(who) : podBaseOfWebId(who) === podBase);
    const token = await verifyPodToken(request, pathname, ctx.verifier);
    // A server that issued the reader's own session can simply read it. On a
    // pod server the page, the session and the pod are one origin, and a token
    // proves nothing the session does not — so opting in cost a round trip to
    // an identity provider that had just said who they were. A Gateway fronting
    // somebody else's pod holds no such session and this does nothing there.
    //
    // One account can hold several WebIDs, so it is the one that owns this pod
    // that is taken, not whichever came back first.
    const session = !token && ctx.webIdsFromSession && sameOriginRequest(request, ctx.frontOrigin)
      ? (await ctx.webIdsFromSession(request).catch(() => [])).find(ownsIt) || null
      : null;
    const webid = token || session;
    if (!webid) return j(401, { error: 'a Solid-OIDC token proving the pod is required' });
    if (!ownsIt(webid)) {
      return j(403, { error: 'the token proves a different pod than the one you listed' });
    }
  ...
  if (pathname === '/.well-known/webfinger') {
    ...
    const out = j(200, jrd({ handle: m[1], host: ctx.host, actor: rec.actorUrl, aliases: podActor,
      page: profilePageOf(rec), subscribe: subscribeTemplate(ctx.frontOrigin) }),
    'application/jrd+json');
    // RFC 7033: a JRD is read from any origin.
  ...
  // The short address a person types: https://<host>/@handle opens the
  // profile page, the way most Fediverse servers spell it. /@handle@pod-host
  // is the same for an account whose address is its pod's, which the front
  // holds under that full address — the form other servers use for a
  // profile that lives elsewhere.
  const at = /^\/@([^/@]+)(?:@([^/@]+))?$/u.exec(pathname);
  if (at) {
    const handle = decodeURIComponent(at[1]).toLowerCase();
    const host = at[2] ? decodeURIComponent(at[2]).toLowerCase() : null;
    const key = host && host !== String(ctx.host).toLowerCase() ? `${handle}@${host}` : handle;
    const rec = await ctx.lookup(key);
    const page = rec && profilePageOf(rec);
    if (!page) return notFound();
    return { status: 302, headers: { location: page, 'cache-control': 'no-store' }, body: '' };
  }
  ...
  // Public documents are read from any origin — a page on another site, the
  // forum's own host among them — the way a pod serves them, so the read
  // carries the open origin and a preflight is answered.
  const open = { 'access-control-allow-origin': '*' };
  if (request.method === 'OPTIONS') {
    return { status: 204, headers: { ...open, 'access-control-allow-methods': 'GET, HEAD, OPTIONS',
      'access-control-allow-headers': 'Accept', 'access-control-max-age': '86400', allow: 'GET, HEAD, OPTIONS' }, body: null };
  }
  if (request.method !== 'GET' && request.method !== 'HEAD') return { status: 405, headers: {}, body: '' };
  // Moved: every old id but the actor's answers with where it is now, so a
  // reply or a boost another server still holds under the old id resolves.
  ...
  // A container is Turtle, a profile page is HTML, everything else here is
  // ActivityPub. Ask for what the thing can be, not for what most of them are.
  const container = podTarget.endsWith('/');
  const page = /\.html?$/u.test(podTarget);
  const accept = container ? 'text/turtle, */*;q=0.8'
    : page ? 'text/html, */*;q=0.8'
      : AP_CT;
  ...
  // server verifying the Move fetches the OLD id and needs a document whose
  // id is that old id — with the same key under it, and `movedTo` saying
  // where the account went. Swapping the new base back to this one gives
  // exactly that; alsoKnownAs and movedTo are set below.
  if (movedBase) text = swap(text, movedBase, base);
  if (up.rest === 'ap/actor') {
    try {
      ...
      if (movedBase) {
        ...
        aka.add(rec.movedTo);
        doc.alsoKnownAs = [...aka];
        const ctxs = [].concat(doc['@context'] || []);
        const declared = (term) => ctxs.some((c) => c && typeof c === 'object' && term in c);
        if (!declared('alsoKnownAs')) ctxs.push({ alsoKnownAs: { '@id': 'as:alsoKnownAs', '@type': '@id' } });
        if (!declared('movedTo')) ctxs.push({ movedTo: { '@id': 'as:movedTo', '@type': '@id' } });
        doc['@context'] = ctxs;
      }
