...
async function route(request, ctx) {
  ...
  if (pathname === '/api/agent' && request.method === 'GET') {
    ...
    for (const webId of webIds) {
      ...
      const about = ctx.agentControl.describe ? await ctx.agentControl.describe({ podBase }).catch(() => null) : null;
  ...
  if (pathname === '/.well-known/webfinger') {
    ...
    Object.assign(out.headers, publicFor(300));
    return out;
  }
  ...
  return { status: 200,
    headers: { ...open, 'content-type': got.type || (container ? 'text/turtle' : page ? 'text/html' : AP_CT), ...publicFor(30) },
    body: text };
}
