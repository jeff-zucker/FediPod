...
import { assertPublicUrl, safeFetch, readCapped } from '../shared/safefetch.mjs';
...
export async function verifyHttpSignature(request, { documentLoader, keyCache, timeWindow } = {}) {
  ...
  // Which signature the sender used: RFC 9421 carries a Signature-Input
  // header beside Signature; the cavage draft carries Signature alone. The
  // library picks the verifier by the same header.
  ...
  if (kind === 'keyFetchError') {
    return {
      verified: false, method, keyId: null, actor: null,
...
//
// JSON.stringify's replacer ARRAY applies at EVERY level, not just the top: the
// old canonical() passed the top-level key names, so `checks` — whose own keys
// are `signature`, `digest`, `dateSkew`, `keyFetched` — serialized as `{}` and
// was covered by no signature at all. Anything nested was equally unstamped.
// Sort recursively instead, and exclude `hmac` only where it actually lives.
...
function canonical(obj) {
  const { hmac, ...rest } = obj;       // eslint-disable-line no-unused-vars
  return JSON.stringify(sortedKeys(rest));
}

// What canonical() used to produce. Kept ONLY for verification, and only until
// every door has been redeployed: a gateway (fedipod.net) and an agent update
// independently, so for a while a receipt signed by an old door has to keep
// verifying against a new drain. Receipts are short-lived — one per inbox item,
// deleted with the item — so this can go once the doors have turned over.
// Nothing signs with it.
...
export function verifyReceipt(receipt, secret) {
  ...
  // The current stamp first; the pre-2026-09-09 one second, for receipts a door
  // that has not been redeployed yet is still writing. See canonicalLegacy.
