...
export async function fillCopy(kv, h, { pod, podFetch, stateUrl, webId = null, log = () => {} }) {
  ...
  if (!await podLease.acquire()) {
    // Held is another device acting on the pod; unreadable is the gateway's
    // own identity refused or unanswered there, which no device has to do with.
    return podLease.denied === 'unreadable'
      ? { ok: false, why: `the pod could not be read (${podLease.unreadable})` }
      : { ok: false, why: 'the account is active on another device' };
  }
