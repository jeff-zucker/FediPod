 ...
 * WebID, which names the browser account here that belongs to it. Returns
 * { handle, rec }, or { error: [status, message] }.
 */
async function accountFor(ctx, address, host) {
  const a = String(address || '').trim();
  if (/^https?:\/\//iu.test(a)) {
    const mine = Object.entries(await ctx.listDirectory?.() || {})
      .filter(([, r]) => r?.webId === a && r.openedAt && !r.movedTo && !r.closedAt);
    // Kept running first: that is the one an app can use.
    const kept = mine.filter(([, r]) => r.keeper);
    const pick = kept.length ? kept : mine;
  ...
  if (!rec?.openedAt || rec.movedTo || rec.closedAt) return { error: [404, 'there is no account with that address here'] };
  return { handle, rec };
}
...
async function route(request, pathname, ctx, deps, log) {
  ...
  if (pathname === '/api/authorize' && request.method === 'POST') {
    ...
    // The owner, proved: not an app checking in, so not made to wait.
    const made = await ensureCopy(ctx, handle, rec, log, { owner: true });
    if (!made.ok) return json(made.status === 409 ? 503 : made.status, { error: made.why });
    const code = rand(24);
