...
export async function pushHeld(ctx, handle, rec, { log = console.log } = {}) {
  ...
  try {
    for (let round = 0; round < 5; round++) {
      const want = (await kv.get(`pushwant/${handle}`))?.text || null;
      n += await readHeld(ctx, handle, rec, { log, takeover: true, waitMs: 20_000 });
      if (((await kv.get(`pushwant/${handle}`))?.text || null) === want) break;
    }
  } finally { await kv.delete(`pushrun/${handle}`).catch(() => {}); }
  return n;
}
...
async function answerForAccount(request, url, ctx, tok, rec, log) {
  ...
  try {
    ...
    // Done acting: the lease goes back, so a browser sitting open takes it
    // up again rather than staying a viewer for its whole term.
    await lease.release().catch(() => {});
    await ctx.noteNext?.(handle, nextDue(store)).catch(() => {});
    reading.delete(handle);
    return handled ? b.response() : new Response(JSON.stringify({ error: 'not found' }), { status: 404 });
...
async function route(request, pathname, ctx, deps, log) {
  ...
  if (!usable(rec) || rec.webId !== tok.webId) return json(401, { error: 'this account is not kept running here any more' });
