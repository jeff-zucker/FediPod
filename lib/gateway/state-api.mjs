...
import { CopyStorage, copyMeta, fillCopy, dropCopy, lockCopy, kvDocFetch, podOnly, safeName, safeHandle } from './copy.mjs';
...
export async function routeStateApi(request, pathname, ctx, deps) {
  ...
  if (pathname === '/api/state/open') {
    ...
    const made = await ensureCopy(ctx, handle, rec, console.log, { owner: true });
  ...
  if (!safeHandle(handle) || (name && !['leave', 'lease.json'].includes(name) && !safeName(name))) return j(400, { error: 'not a state document' });
