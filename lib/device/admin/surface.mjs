...
export function buildAdminSurface({ agent, gate, allowed, log = console.log,
  ...
  const c2s = new C2S({ agent, log, scheme, mount,
    auth: makeC2sAuth({ agent, masto, log, scheme, mount }) });
  ...
  const handler = async (req, res) => {
    ...
    try {
      ...
      // The short address a person types: /@handle opens the profile page.
      const at = /^\/@([^/]+)$/u.exec(p);
      const ownHandle = handle || agent.store?.getConfig?.()?.handle || '';
      if (at && ownHandle && agent.publisher?.urls?.profileHtml
        && decodeURIComponent(at[1]).toLowerCase() === String(ownHandle).toLowerCase()) {
        res.writeHead(302, { location: agent.publisher.urls.profileHtml, 'cache-control': 'no-store' });
        res.end(); return;
      }
      ...
      if (p === '/nodeinfo/2.0') {
        // The schema's own media type, as the NodeInfo spec names it.
        return sendJson(res, 200, nodeinfoDoc({
          version: AGENT_VERSION,
          localPosts: agent.store.countStatuses('post'),
        }), allowed, 'application/json; profile="http://nodeinfo.diaspora.software/ns/schema/2.0#"');
      }
