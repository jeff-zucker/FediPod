...
// `icons` alongside `admin`: the favicons the agent's own pages name. One
// file each, no authority behind them, and the same jail as everything else
// under web/.
const WEB_MOUNTS = ['admin', 'icons'];
...
export function sendJson(res, status, obj, auth, contentType = 'application/json') {
  res.writeHead(status, { 'content-type': contentType, ...securityHeaders(auth, false) });
