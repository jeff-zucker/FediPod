...
export async function post(p, body, ctx, req, res) {   // eslint-disable-line no-unused-vars
  ...
  switch (p) {
    ...
    // The account back in this agent's hands. A viewer publishes nothing and
    // drains nothing, so a lease left behind by something that is gone leaves
    // the account silent with nothing in the interface to say so.
    case '/takeover': {
      if (!await agent.requestTakeover?.()) {
        return json(res, 503, { error: 'the lease could not be taken — the pod refused the write' });
      }
      return json(res, 200, { ok: true, mode: agent.status().mode });
    }
    case '/drain': {
      if (!await agent.requestTakeover?.()) {
