...
// Read every item waiting in another inbox container on this pod through the
// normal intake, then delete it: the container of a root this identity moved
// away from, where a Gateway kept writing until its row was corrected.
export async function drain() {
requireIdentity();
const container = flag('container');
if (!container) { console.error('usage: fedipod drain --container <inbox container url on this pod>'); process.exit(1); }
try {
  const res = await localFetch(HOME, PORT, `/inbox/prune`, {
    method: 'POST', headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ before: new Date().toISOString(), keepConcerning: true, container }),
  });
  const body = await res.json();
  if (res.status >= 400) { console.error(body.error || `HTTP ${res.status}`); process.exit(1); }
  console.log(`${container}: ${body.considered} item(s) — applied ${body.applied}, dropped ${body.dropped}`
    + `, discarded ${body.discarded} unread${body.failed ? `, ${body.failed} failed (left for another run)` : ''}`);
} catch (e) {
  console.error(`agent not reachable on :${PORT} (${e.message})`);
  process.exit(1);
}
