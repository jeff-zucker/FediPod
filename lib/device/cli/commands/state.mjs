...
for (const { name, dir } of homes) {
  ...
  rows.push({ name, pod: pod ? new URL(pod).host : '(no credential)', port: port || '—',
    // A home can hold a forum, which says so itself; anything else that
    // answers is a person unless it says group.
    kind: live?.kind === 'group' ? 'group' : live?.kind === 'forum' ? 'forum' : live ? 'person' : '—',
