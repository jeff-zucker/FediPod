 ...
 */
export async function readPublicDocument(getter, podTarget, { podHome, accept = null, maxBytes = PUBLIC_DOC_MAX_BYTES } = {}) {
  const res = await getter(podTarget, { podHome, accept });
  if (!res || res.status >= 400) return { status: res?.status || 502, text: null };
  // What the pod called it: a front that relabels an HTML page as JSON hands
  // a browser something it will not render.
  const type = res.headers?.get?.('content-type') || null;
  return { status: 200, text: await readCapped(res, maxBytes), type };
}
