...
//
// A second class rather than more of the gateway handler: CSS routes websocket
...
// gateway handler's state by holding the same instance, so a socket is handed
...
import type { FediPodGatewayHandler } from './handler';

export interface FediPodStreamingArgs {
  /** The gateway handler, which owns the running identities. */
  gateway: FediPodGatewayHandler;
}

const STREAMING_PATH = '/api/v1/streaming';

export class FediPodStreamingHandler extends WebSocketHandler {
  private readonly gateway: FediPodGatewayHandler;
  ...
  public async canHandle({ upgradeRequest }: WebSocketHandlerInput): Promise<void> {
    ...
    if (!pathname.startsWith(STREAMING_PATH) || !this.gateway.surfaceFor(host)) {
  ...
  public async handle({ webSocket, upgradeRequest }: WebSocketHandlerInput): Promise<void> {
    const identity = this.gateway.surfaceFor(upgradeRequest.headers.host);
