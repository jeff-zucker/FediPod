// oidc-session.mjs — Solid-OIDC login for a browser page, with a secure,
...
// IndexedDB survives a browser restart and is readable by a service worker,
// and a non-extractable key can sign but never be read out, even by injected
// script.
//
//   import { solidOidcSession } from 'solid-oidc-session/oidc-session.mjs';
//   const oidc = solidOidcSession({ dbName: 'my-app-oidc', clientName: 'My app' });
//
// Page:   const { authorizationUrl } = await oidc.beginLogin({ issuer, redirectUri });
//         location.href = authorizationUrl;                      // ... redirect ...
//         await oidc.completeLogin({ currentUrl: location.href });  // on return
// Anywhere (page or worker, SAME dbName):
//         const session = await oidc.getSession();  // { webId, issuer, fetch, refresh, signOut } | null
//
// No imports, no Node built-ins, nothing named after the application that
// uses it: IndexedDB, WebCrypto and fetch are all it needs.
...
/**
 * One session store. `dbName` is the IndexedDB database the session lives in:
 * a page and its service worker must pass the same name to see the same
 * sign-in, and two apps on one origin pass different names to keep theirs
 * apart. `clientName` is what the pod's login screen shows the person.
 */
export function solidOidcSession({ dbName = 'solid-oidc-session', clientName = 'Solid app' } = {}) {
  function idb() {
  ...
  async function beginLogin({ issuer, redirectUri, clientName: name = clientName, returnTo = null }) {
    ...
    const state = rand(16);
    await idbPut('pending', { issuer, client_id, redirectUri, verifier, state, pair, returnTo,
  ...
  async function completeLogin({ currentUrl }) {
    ...
    // `returnTo` rides only on the handle this call returns: where the person
    // was when they left for the login page, for the caller to send them back.
   ...
   *
   * Dropping the row locally leaves the REFRESH TOKEN alive at the pod for its
   * full life — signing out of a shared or borrowed browser would clear the
   * screen and nothing else, and anyone who had copied the token could go on
   * minting access tokens with it. RFC 7009: hand it back.
   *
  ...
  function sessionHandle(s) {
    ...
    // login without asking them for their address again.
    return { webId: s.webId, issuer: s.issuer, fetch: authFetch, refresh, signOut };
  }

  return { beginLogin, completeLogin, getSession, signOut };
}
